//
// Generated file, do not edit! Created by nedtool 5.3 from messages/ResourceBlock.msg.
//

#if defined(__clang__)
#  pragma clang diagnostic ignored "-Wreserved-id-macro"
#endif
#ifndef __RESOURCEBLOCK_M_H
#define __RESOURCEBLOCK_M_H

#include <omnetpp.h>

// nedtool version check
#define MSGC_VERSION 0x0503
#if (MSGC_VERSION!=OMNETPP_VERSION)
#    error Version mismatch! Probably this file was generated by an earlier version of nedtool: 'make clean' should help.
#endif



/**
 * Class generated from <tt>messages/ResourceBlock.msg:17</tt> by nedtool.
 * <pre>
 * packet ResourceBlock
 * {
 *     int senderId;
 *     int size;
 * }
 * </pre>
 */
class ResourceBlock : public ::omnetpp::cPacket
{
  protected:
    int senderId;
    int size;

  private:
    void copy(const ResourceBlock& other);

  protected:
    // protected and unimplemented operator==(), to prevent accidental usage
    bool operator==(const ResourceBlock&);

  public:
    ResourceBlock(const char *name=nullptr, short kind=0);
    ResourceBlock(const ResourceBlock& other);
    virtual ~ResourceBlock();
    ResourceBlock& operator=(const ResourceBlock& other);
    virtual ResourceBlock *dup() const override {return new ResourceBlock(*this);}
    virtual void parsimPack(omnetpp::cCommBuffer *b) const override;
    virtual void parsimUnpack(omnetpp::cCommBuffer *b) override;

    // field getter/setter methods
    virtual int getSenderId() const;
    virtual void setSenderId(int senderId);
    virtual int getSize() const;
    virtual void setSize(int size);
};

inline void doParsimPacking(omnetpp::cCommBuffer *b, const ResourceBlock& obj) {obj.parsimPack(b);}
inline void doParsimUnpacking(omnetpp::cCommBuffer *b, ResourceBlock& obj) {obj.parsimUnpack(b);}


#endif // ifndef __RESOURCEBLOCK_M_H

